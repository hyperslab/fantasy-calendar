...
class DateFormat(models.Model):
    ...
    def is_reversible(self):
        """
        Return True if any formatted string generated by this date
        ...
        1) The format string must display the date format's time unit
        2) The format string must contain an absolute reference to a
           time unit at or "above" the date format's time unit
        3) The format string must contain enough time units so that the
           time unit of the absolute reference can "connect" to the
